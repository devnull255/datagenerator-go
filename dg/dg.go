 ...
 * ------------  -----------------------  ----------------------------------------------------------------
 * 2020-08-14    Michael Mabin            Addition of map, set and list types, export of data structures
...
func main() {
	...
	var export_cities = flag.Bool("export_cities", false, "Export cities used by datagenerator")
	var export_firstnames = flag.Bool("export_firstnames", false, "Export firstnames used by datagenerator")
	var export_lastnames = flag.Bool("export_lastnames", false, "Export lastnames used by datagenerator")
	var export_streetnames = flag.Bool("export_streetnames", false, "Export streetnames used by datagenerator")
	var export_streettypes = flag.Bool("export_streettypes", false, "Export streettypes used by datagenerator")
...
`)
	...
	if *export_cities {
		...
		cities := g.Cities()
		output := format_output(cities, python_fmt, json_fmt)
	...
	if *export_firstnames {

		firstnames := g.FirstNames()
		output := format_output(firstnames, python_fmt, json_fmt)
	...
	if *export_lastnames {
		...
		output := format_output(lastnames, python_fmt, json_fmt)
	...
	if *export_streetnames {
		streetnames := g.StreetNames()
		output := format_output(streetnames, python_fmt, json_fmt)

		fmt.Println(output)
		os.Exit(0)
	}

	if *export_streettypes {
		streettypes := g.StreetTypes()
		output := format_output(streettypes, python_fmt, json_fmt)
